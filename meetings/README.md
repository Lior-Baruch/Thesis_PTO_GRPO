...
> and the reason none of them is updated when the data moves. Several are now stale against
> [`../STATUS.md`](../STATUS.md): the decks built before 2026-08-18 describe the GRPO K=5 arm as
> having **one** iteration (it has five, scored 0–5 on both graders), quote the held-out grid as
> **22,272 cells / 29 model states** (now 39 × 8 × 96 = **29,952**), and predate the compute axis
> entirely. **Before re-presenting any slide from an existing builder, re-check its numbers against
> `STATUS.md` and the rendered tables** — or copy the builder to a new dated one and update there.
> Do not edit a past deck in place; it is a record of what the supervisor was actually shown.
...
- **`build_status_deck.py`** — the **whole project** in one deck, for a room where not everyone has
  read the drafts. Background and setup, the four arms and what is scored, the main PTO-vs-GRPO
