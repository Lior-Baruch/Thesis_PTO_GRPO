...
| **Paper drafts + the claim→artifact ledger** | [`papers/README.md`](papers/README.md), then each paper's `README.md` + **`NUMBERS.md`**. **One live draft → ARR October 2026, iterations-only** (2026-09-04): [`papers/2026_grpo_lookahead_mi/`](papers/2026_grpo_lookahead_mi/) (*GRPO with Look-Ahead in Motivational Interviewing* — the two GRPO arms as data; PTO cited as origin, never data). The 2×2 (*Same Lever, Different Optimizer*, retired 2026-09-04), the ICLR-format P1 and every earlier Exp3 draft are under `papers/archive/` (tracked). The EDA's `lookahead/` and `compute/` families own the cross-K numbers; the retired *Same Lever* draft's `analysis/out/` + `tables/` stay the EDA self-check's frozen fixture. ⚠ The live draft is **two-way synced to an Overleaf project the supervisors can edit** — `overleaf.py pull` BEFORE editing it, `push` after; build with `build.py`, never the raw pdflatex chain | per draft |
