...
**$0 in API** — the constraint that stopped GRPO_LA5 in Exp3. The therapist is also selectable
...
| **Paper drafts + the claim→artifact ledger** | [`papers/README.md`](papers/README.md), then each paper's `README.md` + **`NUMBERS.md`**. **The single live draft: [`papers/2026_pto_grpo_mi/`](papers/2026_pto_grpo_mi/)** (*Same Lever, Different Optimizer* — the full 2×2, ARR October 2026; **iterations-only axis**, decided 2026-08-27; planned as P2 in [`papers/BRAINSTORM_2026-08-25.md`](papers/BRAINSTORM_2026-08-25.md)). P1 (*Scoring the Continuation*, GRPO-only, ICLR plan dropped 2026-08-27) and every earlier Exp3 draft are retired to `papers/archive/` (tracked). The EDA's `lookahead/` and `compute/` families own the cross-K numbers; the retired *Same Lever* draft's `analysis/out/` + `tables/` stay the EDA self-check's frozen fixture | per draft |
