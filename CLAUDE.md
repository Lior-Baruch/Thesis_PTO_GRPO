...
- **Next step:** cheapest RQ-i point = one generate-only pass with the existing PTO LA5 iter-5
  ...
  - **Durable LA5-resume facts** (what's actually on Drive; dated forensics in
    ...
    **PTO LA5** has trained adapters for iters 1–5 but only I1–I4 scored — the iter-5 eval convs
    were **never generated** (`model_iter_5` conv dir exists but is EMPTY; `iteration_6/` stopped at
    `pref_pairs`, no adapter). **GRPO LA5**: iter-1 adapter trained AND scored; its `iteration_2/` is
