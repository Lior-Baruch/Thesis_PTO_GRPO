...
R:Q/%CR/%MICO). See [`tables/eval/leaderboard_scorecard.md`](tables/eval/leaderboard_scorecard.md)
for the one-glance arm × metric scorecard.

## 2. Headline (RQ-ii, K=0) — PTO ahead at the matched endpoint
See [`figures/headline/`](figures/headline/) and
...
- **PTO_LA0 Q1+Q2 3.00 → 4.26** (dz 1.43, *large*); **GRPO_LA0 peaks 4.08 @ iter 8 then regresses
  ...
  [`tables/stats/PTO_vs_GRPO_LA0_paired.md`](tables/stats/PTO_vs_GRPO_LA0_paired.md).
- OLS slope PTO 0.120/iter vs GRPO 0.072/iter
  ([`tables/stats/Q1Q2_slope_by_arm.md`](tables/stats/Q1Q2_slope_by_arm.md)).
...
See [`figures/eval/factor_loadings.png`](figures/eval/factor_loadings.png) and
[`figures/eval/behavior_drift.png`](figures/eval/behavior_drift.png).
...
([`tables/stats/PTO_K0_vs_K5_paired.md`](tables/stats/PTO_K0_vs_K5_paired.md)). On reward
...
- Degeneration gate clean across arms (leak ≈ 0, loops eliminated) —
  [`figures/training/`](figures/training/).
- Short-reward faithfulness at MCL=12: GRPO grows more faithful with length (≈0.86 → 0.94), PTO less
  (≈0.87 → 0.76) — [`figures/reliability/`](figures/reliability/).
- PTO preference probe is real (`wins_correct` 0.65 → 0.71) — [`figures/preference/`](figures/preference/).
