...
- The lever's sign is a **function of budget**, not a constant: GRPO K=5 is clearly worse at
  <=18 GPU-h and only draws level at ~23-27 (`compute/cost/tables/budget_sweep_GRPO_K_<judge>.md`).

**No sentence about look-ahead is complete without naming its axis.** See `eda_analysis/compute.py`
and the `compute/cost/` artifacts.
...
per-session Δ transposed onto the turn-count row — see `lookahead/behaviour/tables/k_paired_channels.md`.)*
