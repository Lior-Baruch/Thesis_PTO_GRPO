> **Excerpt — first 60 of 1,584 rows.** The full table is too large to read as markdown, so it lives on sheet `k_paired_channels` of the `.xlsx` workbook in this folder. Load it with `pandas.read_excel(..., sheet_name="k_paired_channels")`.
