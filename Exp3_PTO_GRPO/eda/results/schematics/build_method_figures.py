...
These are schematics, not results: nothing here reads the data. Since the 2026-08-18 EDA reorg
they live at ``eda/results/schematics/`` (moved from ``Exp3_PTO_GRPO/figures/``) as the one
judge-less, notebook-less family — ``tools/render_results.py`` never touches this folder and it is
listed in ``exports.PRESERVE``, so ``reset_results`` / ``build_index`` cannot delete or re-index
it. Outputs are written NEXT TO THIS SCRIPT (``HERE``), whatever the cwd. They are consumed by the
meeting decks and the paper's ``sync_figures.py`` and are meant to be reused in the thesis.

Run (from eda/results/schematics/):  & ..\..\..\..\.venv\Scripts\python.exe build_method_figures.py
"""
import os
import matplotlib
matplotlib.use("Agg")
