...
| **Responsiveness** `refl_after_ct`, `pra_after_st`, `pers_after_st`, `refl_after_st`; table `responsiveness_<judge>` = P(therapist code \| preceding patient code) | what the policy does after change talk vs sustain talk — reflecting change talk is the core MI skill; praising sustain talk is its purest failure. The three answers to sustain talk (praise, persuasion = the righting reflex, reflection = MI's recommended answer) are per conversation over the therapist turns that follow a sustain-talk utterance, so read them together: one policy can avoid praise by persuading instead |
