...
| `lookahead/mechanism.ipynb` | **why** — the K-mechanism chain, dispersion, reward faithfulness at matched policy, tail audit | `k_mechanism_overpraise_chain` (+ fig `k_mechanism_overpraise`) · the GRPO companions `k_mechanism_overpraise_chain_grpo` + `praise_premium_grpo` (does the training reward pay for praise, net of how common it is — `pref.feature_premium`; + fig `praise_premium_grpo`) · `dispersion_{by_iter,ratios,tau,expectation}` (+ figs `dispersion`, `dispersion_tau`) · `faithfulness_{curve,curve_heldout,curve_by_iter,k_by_iter,matched_policy,matched_policy_tests,k_summary,by_coop,levels,levels_rho}` (+ figs `faithfulness`, `faithfulness_heldout`) · the tail-audit tables (by iter, cues, within group, score by realized turns) + fig `tail_audit` |
...
| `lookahead/text.ipynb` | **what each policy learned / unlearned, in the TEXT** — the eval conversations embedded (MiniLM, same space as the training-side probe): base-repertoire clustering + occupancy, centroid drift + cross-arm cosines + update-direction alignment, diversity / persona sensitivity, judge-free responsiveness proxies (cross-checked against MITI reflections under both graders), patient-side shape, within-session profiles | `repertoire_{clusters,by_state,learned_unlearned,stability}` · `drift_{by_state,cosines}` · `update_alignment` · `diversity_by_state` · `template_similarity_by_turn` · `text_levels` · `k_text_{paired,summary}` · `echo_validation{,_pooled}` · `session_profile` · `profile_kcontrast` · figs `repertoire_occupancy`, `repertoire_learned_unlearned`, `drift`, `diversity`, `text_levels`, `k_text_forest`, `session_profile` · `text_numbers.json`. Backing module `eda_analysis/text.py` (+ `plotting/text.py`); embeddings cached in `eda/.emb_cache/eval/` |
| `lookahead/process.ipynb` | **utterance-level MI process** — the `MIPROC` coder (one MITI/MISC-style code per therapist AND per patient utterance): code mix, persona-paired K contrast on every process metric, yield P(change talk \| therapist code), responsiveness P(therapist code \| patient code), within-session change talk, time to first change talk, and parity vs the same grader's MITI/PCT counts. One table/figure per grader that has MIPROC on disk | `process_levels_<judge>` · `k_process_{paired,summary}` · `yield_<judge>` · `responsiveness_<judge>` · `ct_trajectory_<judge>` · `time_to_change_talk` · `parity{,_pooled}_<judge>` · figs `code_mix_<judge>`, `k_process_forest_<judge>`, `yield`, `transition_endpoints`, `ct_trajectory` · `process_numbers.json`. Backing module `eda_analysis/process.py` (+ `plotting/process.py`) |
| `lookahead/shared_base.ipynb` | **the GRPO look-ahead contrast on ONE shared Base** — the GRPO paper's view (2026-09-24): both GRPO runs' iteration-0 draws of the same untrained model pooled into one Base (192 conversations, two per persona), under both graders; K contrasts start at iteration 1 (Holm across 1..10). Every Base-dependent number the paper quotes, plus the complete score table and the change-talk **persistence** analysis (P(patient change talk \| the patient's previous code), pooled and per conversation, persona-paired). GRPO arms only | `levels_long` · `k_contrast` · `significant_iterations` · `gains` · `score_table_<judge>` · `process_levels_<judge>` · `k_process_paired` · `yield_<judge>` · `responsiveness_<judge>` · `ct_trajectory_<judge>` · `persistence_<judge>` · `persist_levels_<judge>` · `k_persistence` · `cond_yield_<judge>` · `parity{,_pooled}_<judge>` · `text_{drift,drift_cosines,diversity}` · `marker_and_length` · `sd_by_iter` · `sd_trend` · `agreement_by_state` · `agreement_summary` · `state_pairs` · `sign_preservation` · `judge_offset` · figs `levels_grid_<judge>`, `persistence` · `shared_base_numbers.json`. Backing module `eda_analysis/shared_base.py` |
...
1. **`scoring/Run_Eval.ipynb`** — async oracle scoring → the score lake,
   ...
   `QUESTIONNAIRE_FILTER=["MIPROC"]`. It is scored on the **GRPO arms only** so far (2 × 11 × 96 =
   2,112 conversations, primary grader); `lookahead/process` renders whichever graders have it.
...
- **`shared_base`** (→ `lookahead/shared_base`) — the GRPO contrast on ONE shared Base: `share_base`
  (``mode="per_arm"`` gives every arm of a method the pooled base draws as its iteration 0;
  ``mode="single"`` attaches them to the first arm only — the 21-state view for statistics across
