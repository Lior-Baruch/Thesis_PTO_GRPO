...
all Holm $p\approx0$. \dz{} is Cohen's $d$ on paired differences with the conventional
$0.2/0.5/0.8$ banding. \textbf{MICI is scored so lower is better}, so its large positive \dz{}
is a deterioration --- the one row contradicting the rest.
\textbf{Bottom:} the head-to-head, reported \emph{both} ways because comparing only final
iterations is unfair to GRPO. PTO leads on the reward under either reading, but the effect
halves at best-vs-best and \textbf{the MITI and MICI gaps vanish} ($p=0.57$, $0.52$) --- the
