...
Three further results give the interaction its practical meaning. \textbf{The flip is
grader-conditional at the top} (\S\ref{ssec:winners}): the training oracle scores \GRPO-$\Kf$'s
endpoint as the best final state in the grid, but under the held-out judge that endpoint is
statistically indistinguishable from \PTO-$\Kz$'s --- own-base gains of $+1.038$ vs.\ $+1.036$
rubric points, a paired null that replicates across two evaluation draws --- so the two winning
cells separate only under the grader that supplied the training reward, exactly where
\S\ref{sec:measurement} shows that grader's per-conversation range collapsing.
\textbf{The horizon decides what is
...
confirmed by two graders and replicated evaluation draws; (ii) a measurement analysis showing
that the training grader's per-conversation discriminative range collapses precisely at the
winning look-ahead checkpoints --- one-sided saturation that a held-out judge exposes, with a
cheap diagnostic any judge-trained pipeline can run; (iii) evidence from a judge-free transcript statistic that
