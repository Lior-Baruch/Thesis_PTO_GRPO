...
| [`2026_pto_grpo_mi/`](2026_pto_grpo_mi/) | *Same Lever, Different Optimizer: The Reward-Horizon $\times$ Optimizer Interaction in Simulated Motivational Interviewing* | Exp3, **all four arms** — the 2$\times$2, both graders, behaviour, ICLR-era regime re-scoring. **Iterations-only axis** (2026-08-27): no GPU/budget analysis; the data-per-iteration asymmetry is a Limitations disclosure | **THE submission — the single live paper; targeting the ARR October 2026 cycle** (submission 2026-10-12, commitment 2026-12-20; feeds NAACL 2027 + COLING 2027, venue chosen in December from reviews) |
| [`archive/2026_grpo_lookahead_mi/`](archive/2026_grpo_lookahead_mi/) | *Scoring the Continuation: $K$-Turn Look-Ahead Rewards for Group-Relative Policy Optimization in Motivational Interviewing* | Exp3, the two GRPO arms only (rescoped 2026-08-26 to GRPO-only, scores-not-deltas; 9-page ICLR body met) | **retired 2026-08-27** — the supervisors chose a single ARR submission and dropped the ICLR 2027 plan; its GRPO-only argument survives as §§4/6/8 of `2026_pto_grpo_mi`. Draft was complete and building; its `NUMBERS.md` + `*_grpo` EDA artifacts remain valid |
...
**One live paper** (decided with the supervisors 2026-08-27): [`2026_pto_grpo_mi/`](2026_pto_grpo_mi/)
— **P2** of [`BRAINSTORM_2026-08-25.md`](BRAINSTORM_2026-08-25.md) (the cold table read that
ranked five candidate papers) — targeting the **ARR October 2026 cycle** (submission 2026-10-12,
commitment 2026-12-20; the single cycle feeds NAACL 2027 + COLING 2027, and the venue is chosen
in December once reviews exist). It is the full $2\times2$, where the optimizer ranking flips
with the reward horizon; it owns the interaction, the two-winners head-to-head, the
two-optimizer behaviour comparison, the ICLR-era regime re-scoring, and the full-grid
measurement section. **Its comparison axis is iterations only** (2026-08-27): the GPU-hour /
budget analyses stay in the EDA (`results/compute/cost/`), out of the paper; the
matched-iterations≠matched-data asymmetry is disclosed in its Limitations.

**P1** ([`archive/2026_grpo_lookahead_mi/`](archive/2026_grpo_lookahead_mi/), the GRPO-only
look-ahead paper) was archived the same day and its ICLR 2027 plan dropped — a complete,
building draft, retired by a scope decision rather than a defect.
...
live draft carries `acl.sty` + `acl_natbib.bst`; the archived P1 carries
