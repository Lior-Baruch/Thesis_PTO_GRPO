...
graders. All comparisons are made at \emph{matched iteration counts}; an iteration is 96 fresh
conversations for every arm, but the two optimizers extract different amounts of oracle-scored
training signal from it by construction, so a matched-iteration verdict is not a matched-data
one (Limitations).
