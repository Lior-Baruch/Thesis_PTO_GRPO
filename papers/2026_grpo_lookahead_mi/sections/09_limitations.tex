...
iterations each process contrast is significant rather than at iteration 10 alone. We regard this as the principal limitation; it is distinct from, and not bounded
...
96-conversation sample (the Base on two, which we pool). For the $\Kf$ policy at iteration 10, the
re-draw of \S\ref{sec:reward} differs from the original by at most $|\dz|\,0.174$ across 9 instruments and
...
keep the session open that a turn-level reward does not, and the $\Kf$ policy's longer sessions
at iteration 10 ($31.9$ vs.\ $25.2$ utterances, $\dz\,0.43$) are consistent with it. Two runs
cannot separate this from the horizon effect proper.

\paragraph{The patient's replies are inside the look-ahead reward.}
The $\Kf$ reward is given after the patient simulator has replied to the candidate (and twice more
inside the rollout), while the $\Kz$ reward is given before any reply. The patient-side results of
\S\ref{sec:patient} therefore partly measure what the $\Kf$ reward was optimising rather than an
outcome outside it, and the replies come from the same model that gives the reward, so the $\Kf$
run may have learned what moves this simulator in particular. Evaluating with a patient simulator
from another model family would separate the two.
...
and mean turn length roughly tripled over training in both runs, so many late turns end
