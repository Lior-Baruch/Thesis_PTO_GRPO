...
and we cannot separate ``look-ahead helps'' from ``this look-ahead run went well.'' The $K{=}0$
arm's decline over its last two iterations and its late over-praise spike are likewise events of
one run, which is why \S\ref{sec:reward} reports the contrast against that arm's best checkpoint as
well as its last, and why \S\ref{sec:therapist}--\ref{sec:patient} report at how many of the ten
matched iterations each process contrast clears Holm rather than the endpoint alone. We regard this as the principal limitation; it is distinct from, and not bounded
...
At the untrained base, the two arms' independent draws of the identical policy differ on \QQ\ by
$\dz\,0.115$ under the training oracle and $0.043$ under the held-out judge, neither significant
(Table~\ref{tab:byiter}); at the trained $\Kf$ endpoint the re-draw of \S\ref{sec:reward} differs
from the original by at most $|\dz|\,0.174$ across 9 instruments and 2 graders, nothing
significant. The remaining states are single draws, and a replicate bounds evaluation noise only,
never the training variance above.

\paragraph{Matched iterations are not matched cost.}
An iteration is 96 conversations and $G{=}8$ scored candidates per prompt in both arms, so oracle
...
equalise it. Read at matched GPU-hours instead, with each arm at its best checkpoint within the
budget under the selecting grader, the comparison is budget-dependent: at about 13 GPU-hours the
turn-level arm is ahead on \QQ\ ($\dz\,-0.74$ under the training oracle, $-0.78$ held out), at
about 23 GPU-hours the two are level under the training oracle ($\dz\,0.07$, n.s.) and look-ahead
leads under the held-out judge ($\dz\,0.33$, $p_{\text{holm}} = .012$), and beyond the turn-level
arm's total budget the comparison is the best-checkpoint one of \S\ref{sec:reward}. GPU-hours are
...
sessions ($31.9$ vs.\ $25.2$ utterances, $\dz\,0.43$) are consistent with it. Two arms cannot separate this from the
...
finding is the most robust behavioural claim here precisely because a deterministic lexical marker
corroborates it without a model in the loop. Therapist responses are also capped at 200 tokens,
and mean turn length roughly tripled over training in both arms, so many endpoint turns end
mid-sentence (Appendix~\ref{app:example}); the cap is identical across arms and the truncated turn
is what the patient simulator and both graders saw, so it does not bias the contrast, but it is
part of what every score in this paper measures.

\paragraph{Instruments and the process coder.}
...
evidence, and Appendix~\ref{app:saturation} quantifies what happens to the trained-against
grader's discriminative range over a long run. The MITI-style coder is the least dependable
instrument at the arm level, and we do not lean on its differences alone. The process coder of
...
codes reproduce the session-level change-talk counts of the same grader (pooled $\rho$
$0.88$--$0.95$ across the 22 states), but its therapist-side codes reproduce the MITI behaviour
...
one the two graders apply differently to the look-ahead policy (\S\ref{sec:therapist}), and no
