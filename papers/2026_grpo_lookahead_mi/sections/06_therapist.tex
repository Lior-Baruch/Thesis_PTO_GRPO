...
the behavioural difference between the two methods; this section and the next quantify it with
...
\textbf{The two policies learn different behaviours.} Initially, the base policy's turns are mostly
information giving and persuasion, with open questions, simple reflections and the occasional
affirmation making up the rest (Figure~\ref{fig:process}a--b, iteration 0). Under turn-level [k=0?]
...
something else) and grow their turns to the 200-token cap (see Limitations). The $\Kf$ policy also
...
therapist turn with a sentence encoder [which one] and following each policy's centroid, the two arms move
...
$0.19$ under $\Kz$ and from $0.41$ to $0.30$ under $\Kf$. [show diagram, explain]
