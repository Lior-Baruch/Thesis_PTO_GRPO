...
is not trivial: a single rollout is a one-sample Monte Carlo estimate of where a turn leads, and
where PTO's pair selection filters that noise, GRPO trains on every candidate
(\S\ref{sec:method}). We ask whether the look-ahead reward retains its benefit under
group-relative optimisation, and how it shapes the policy over ten iterations.
