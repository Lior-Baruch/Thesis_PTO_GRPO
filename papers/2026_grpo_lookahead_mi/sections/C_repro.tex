...
the arm's own name, adapter repository, two output paths, and start timestamp.
...
Look-ahead $K$ & 0 ($\Kz$ arm) / 5 ($\Kf$ arm) \\
Optimizer steps, 10 iterations & $1{,}128$ ($\Kz$) / $1{,}070$ ($\Kf$) \\
\bottomrule
\end{tabular}}
\caption{Resolved configuration, identical across the two arms except the final two rows.
Source: each arm's \texttt{run\_metadata.json}; the step counts are read from the per-step
...
\midrule
process coder & this work & one code per therapist and per patient utterance & \S\ref{sec:therapist}--\ref{sec:patient} \\
\bottomrule
\end{tabular}
\caption{The eight instruments and the process coder. \QQ\ is the mean of the Q1 and Q2 means. CT
and ST are the patient's change-talk and sustain-talk utterance counts. MICI is lower-is-better;
its per-channel counts and rates are the material of \S\ref{sec:therapist}. The process coder is
...
\subsection{The utterance-level process coder}
\label{app:coder}

The process coder is administered as one further call per conversation under each grader, with the
same rubric-first layout. The grader is addressed as an MI process coder trained on the MITI
...
conversations is coded under both graders; three held-out rows that came back one code short on
...
patient turn $i-1$; the yield of a code is the share of its turns whose patient reply is change
talk, and the responsiveness rates condition on the patient's preceding code. As a check of the
coder against the session-level instruments of the same grader, the per-utterance codes summed
per conversation are compared with the MITI behaviour counts and the PCT change-talk counts:
across the 22 model states, the patient-side counts agree closely (Spearman $\rho$ pooled within
...
long turn performs, the process coder its dominant one, which is why the MITI coder reports
$5.5$ questions per conversation where the process coder reports $1.7$ question-turns under the
training oracle. \S\ref{sec:therapist} therefore quotes ``open-question turns'' and
\S\ref{sec:reward} ``questions per turn'' with their units.
...
The marker of \S\ref{sec:therapist} is one case-insensitive regular expression matched against
...
Model states are scored offline into a score archive keyed by grader, replicate, instrument and
model, one record per conversation; an interrupted scoring run resumes by skipping completed work.
The reported draw is replicate 0 for each grader.
...
iterations of each arm), and the optimizer-step counts are the number of per-step training
artifacts each arm wrote. The GPU-hour totals in the Limitations and the Ethics Statement and the
...
code release, through a claims ledger released with it. Figures~\ref{fig:grid-primary}
and~\ref{fig:grid-heldout} are copied from that tree; Figures~\ref{fig:schematic},
\ref{fig:headline}, \ref{fig:process}, \ref{fig:overpraise}, \ref{fig:process-heldout},
\ref{fig:responsiveness}, \ref{fig:textspace}, \ref{fig:forest} and~\ref{fig:tails} are drawn
from its tracked tables by scripts in the paper folder, at the width each is included at. The
