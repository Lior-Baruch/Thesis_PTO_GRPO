...
compared within one (arm, iteration, conversation-length) cell, that the training reward orders the
...
10 iterations under both graders. By prefix length, agreement at the shortest admitted prefix (12
...
oracle, 17 of 20 favour $\Kf$ under the held-out judge. An effect whose sign depends on the grader
is not distinguishable from zero. The matched-policy cut is also the weakest-powered one (a single
model state per arm, and only approximately matched, since the policy drifts within the iteration
...
coincides with the iteration in which its over-praise rate jumps (the judge-free marker goes
from $0.093$ to $0.671$ between iterations 9 and 10), which is what a group containing both
over-praising and non-over-praising candidates would do under a grader that rewards praise: the
